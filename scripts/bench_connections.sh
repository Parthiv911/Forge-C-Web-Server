...
# Sweep these client concurrencies
CONNS_LIST=(${CONNS_LIST:-1 2 4 8 16 32 64 128})
...
for CONNS in "${CONNS_LIST[6]}"; do
  echo "Benchmarking $FILE with THREADS=$THREADS CONNS=$CONNS ..."

  RES=$("$WRK" -t"$THREADS" -c"$CONNS" -d"$DUR" --latency "$URL/$FILE")

  echo "===== RAW WRK OUTPUT for $FILE c=$CONNS ====="
  echo "$RES"
  echo "============================================="
  ...
  SIZE=$(stat -c%s "public/$FILE" 2>/dev/null || stat -f%z "public/$FILE")

  echo "$FILE,$SIZE,$THREADS,$CONNS,$DUR,$REQS,$RPS,$LAVG_MS,$LSTDEV_MS,$LMAX_MS,$P50_MS,$P75_MS,$P90_MS,$P99_MS,$IMPLIED_MS" >> "$CSV"
done
